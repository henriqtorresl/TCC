...
export async function POST(
  ...
  try {
    ...
    const payload = await chatService.finalizeAttendance(String(patient.id), id);
    ...
    if (
      error instanceof Error &&
      (error.message === "attendance_already_completed" ||
        error.message === "attendance_close_failed")
    ) {
      return NextResponse.json({ error: error.message }, { status: 409 });
    }
    console.error("Unhandled error in /api/attendances/[id]/finalize:", error);
