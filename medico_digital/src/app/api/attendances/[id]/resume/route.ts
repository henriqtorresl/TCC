...
export async function POST(
  ...
  try {
    ...
    const payload = await chatService.resumeAttendance(String(patient.id), id);
    ...
    if (
      error instanceof Error &&
      (error.message === "attendance_already_active" ||
        error.message === "attendance_reopen_failed")
    ) {
      return NextResponse.json({ error: error.message }, { status: 409 });
    }
    console.error("Unhandled error in /api/attendances/[id]/resume:", error);
